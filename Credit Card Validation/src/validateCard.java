...
public class validateCard {
	...
	public static boolean cardTest(long cardNumber) throws IOException {
		...
		// Step 1. Double alternating digits starting with the first digit in the
		// sequence.
		for (int i = 0; i < 16; i += 2) {
			digits[i] = digits[i] * 2;
		// Step 2. If the doubling resulted in a number with two digits, add them
		 // together to get a single digit number
			if (digits[i] >= 10) {
				digits[i] = (digits[i] / 10) + (digits[i] % 10);
			}
		}

		// Step 3. Now go back to the original credit number and replace the digits that
		// you doubled with the new value � either the doubled value, or the doubled
		// value with the digits added together � and add it all up.
		// Step 3 is actually not necessary, because I just need to make a sum and test
		// the sum.
		// Step 4. Check to see if the sum is evenly divisible by 10 (you can simply
		// look to see whether or not it ends with a zero).
		int sum = 0;
	...
	public static void loopCards() throws IOException {
		...
		System.out.println("There are " + falseCards + " invalid card numbers.");

	}

	
	//Steps for testing cards
